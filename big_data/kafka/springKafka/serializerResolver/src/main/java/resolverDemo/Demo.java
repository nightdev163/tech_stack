...
public class Demo {
    ...
    @KafkaListener(id="catConsumer", topics="resolverDemoCat")
    public void listen(Cat cat) {
        String str = "name " + cat.getName() + " age" + cat.getAge();
        System.out.println("Received " + str);
    }

    @KafkaListener(id="hatConsumer", topics="resolverDemoHat")
    ...
    public ApplicationRunner runner(KafkaTemplate<String, Cat> template) {
        return args->template.send("resolverDemo",  new Cat("this is cat", 3));
    }

    // 已经可以发送
    @Bean
    public ApplicationRunner runner2(KafkaTemplate<String, Hat> template) {
        return args->template.send("resolverDemo",  new Hat("this is hat", 5));
    }
}
